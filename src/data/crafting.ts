...
/**
 * Лестница сфер разбора: максимум ОДНА сфера с предмета.
 * Роллы сверху вниз (сначала ранг выше своего, потом свой, потом ниже):
 * первое срабатывание забирает приз. Шансы в %.
 */
export const DISASSEMBLE_SPHERE_LADDER: Record<string, [string, number][]> = {
  'Обычный': [['Редкий', 1], ['Обычный', 5]],
  'Редкий': [['Раритетный', 1], ['Редкий', 5], ['Обычный', 10]],
  'Раритетный': [['Эпический', 1], ['Раритетный', 5], ['Редкий', 10], ['Обычный', 15]],
  'Эпический': [['Смертоносный', 1], ['Эпический', 5], ['Раритетный', 10], ['Редкий', 15], ['Обычный', 20]],
  'Смертоносный': [['Легендарный', 1], ['Смертоносный', 5], ['Эпический', 10], ['Раритетный', 15], ['Редкий', 20], ['Обычный', 25]],
  'Легендарный': [['Божественный', 1], ['Легендарный', 5], ['Смертоносный', 10], ['Эпический', 15], ['Раритетный', 20], ['Редкий', 25], ['Обычный', 30]],
  'Божественный': [['Божественный', 5], ['Легендарный', 10], ['Смертоносный', 15], ['Эпический', 20], ['Раритетный', 25], ['Редкий', 30], ['Обычный', 35]],
};

/** Ролл одной сферы с предмета по лестнице. Возвращает ранг сферы или null. */
export function rollLadderSphere(quality: string): string | null {
  const ladder = DISASSEMBLE_SPHERE_LADDER[quality];
  if (!ladder) return null;
  for (const [rank, chance] of ladder) {
    if (Math.random() * 100 < chance) return rank;
  }
  return null;
}
