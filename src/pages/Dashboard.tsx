...
const debugAddMods = (count: number) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Редкость усиливает оба параметра.
    const qmult = QUALITY_MOD_MULT[q.name] || 1;
    if (qmult !== 1) {
      for (const k of Object.keys(stats)) {
        stats[k] = Math.round(stats[k] * qmult * 10000) / 10000;
      }
    }
    addItem({
...
export const Dashboard = () => {
  ...
  const statCapsules: StatInfo[] = [
    ...
    { label: 'BLK', value: `${stats.block.toFixed(1)}%`, desc: 'Блок. Прямой шанс (в процентах). При срабатывании — 100% поглощение.', breakpoints: ['10 блока → 10%', '25 → 25%', '50 → 50% (кап)'] },
