...
// ---------------------------------------------------------------------------
// getAutoBranch — full version (8 event types)
// ---------------------------------------------------------------------------
function getAutoBranch($template, $zone) {
    if (isset($template['noAutoBranch']) && $template['noAutoBranch']) return null;
    if (isset($template['branch'])) return null;
    $t = $template['type'] ?? 'neutral';
    $zoneLabel = $zone ? $zone : 'окрестностей';
    if ($t === 'combat') {
        $r = ['Железо', 'Инструменты', 'Пластмасса', 'Дерево', 'Изолента'];
        return ['prompt' => '', 'outcomes' => [
            ['text' => "Даём бой и выходим победителями из схватки у $zoneLabel.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,3),'itemCount'=>rangeInt(1,2)];}, 'resourceCost' => $r[0], 'resourceText' => "Используя [$r[0]] ремонтируем оружие в бою — трофеи x3.", 'noResourceText' => "Без [$r[0]] ствол заклинило — обычная награда.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,3),'itemCount'=>rangeInt(1,2)];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Тактически отступаем, уводя противника в ловушку. Возвращаемся за трофеем.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,3)];}, 'resourceCost' => $r[1], 'resourceText' => "[$r[1]] помогли замаскировать ловушку — лут x2.", 'noResourceText' => "Без [$r[1]] ловушка слабая — часть добычи потеряна.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,3),'exp'=>E($l,2)];}, 'noResourceEffects' => function($z,$l){return ['chips'=>NC($l,1)];}],
            ['text' => "Вовремя подоспевшая помощь — патруль союзников разгоняет врагов.", 'weight' => 20, 'effects' => function($z,$l){return ['healPercent'=>RF(0.02,0.06),'exp'=>E($l,4),'chips'=>C($l,3),'itemCount'=>1];}, 'resourceCost' => $r[2], 'resourceText' => "Отблагодарили патруль [$r[2]] — союзники поделились боеприпасами. Награда x2.", 'noResourceText' => "Нечем отблагодарить — [$r[2]] нет. Разошлись сухо.", 'resourceEffects' => function($z,$l){return ['itemCount'=>rangeInt(1,2),'exp'=>E($l,3)];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Противник оказывается сильнее. Получаем серьёзные ранения в бою.", 'weight' => 20, 'effects' => function($z,$l){return ['damagePercent'=>0.10,'chips'=>NC($l,2)];}, 'resourceCost' => $r[3], 'resourceText' => "[$r[3]] смягчили удар — ранения лёгкие.", 'noResourceText' => "Противник сильнее, [$r[3]] нет — урон удвоен, ранения тяжёлые.", 'resourceEffects' => function($z,$l){return ['damagePercent'=>-0.05];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>0.10];}],
            ['text' => "Прорываемся с боем, теряем часть припасов, но приобретаем боевой опыт.", 'weight' => 15, 'effects' => function($z,$l){return ['damagePercent'=>RF(0.02,0.05),'exp'=>E($l,4)];}, 'resourceCost' => $r[4], 'resourceText' => "Повезло, [$r[4]] поймало пулю — урон -50%.", 'noResourceText' => "Прорываемся с боем — [$r[4]] нет, урон x2, но опыт получили.", 'resourceEffects' => function($z,$l){return ['damagePercent'=>-RF(0.01,0.02)];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>RF(0.02,0.05)];}],
        ]];
    }
    if ($t === 'loot') {
        $r = ['Инструменты', 'Батарейки', 'Топливо', 'Изолента', 'Консервы'];
        return ['prompt' => '', 'outcomes' => [
            ['text' => "Находка в целости — всё, что здесь было, достаётся нам.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,4),'exp'=>E($l,2),'itemCount'=>rangeInt(1,2)];}, 'resourceCost' => $r[0], 'resourceText' => "Благодаря [$r[0]] добираемся до тайника первыми — лут x3.", 'noResourceText' => "Без [$r[0]] упаковка не вскрыта — часть пропала.", 'resourceEffects' => function($z,$l){return ['itemCount'=>rangeInt(1,2),'chips'=>C($l,3)];}, 'noResourceEffects' => function($z,$l){return ['chips'=>NC($l,0)];}],
            ['text' => "Помимо основного, замечаем скрытый тайник в $zoneLabel.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,3),'itemCount'=>rangeInt(1,2)];}, 'resourceCost' => $r[1], 'resourceText' => "[$r[1]] помогают вскрыть тайник без шума — находка x2.", 'noResourceText' => "Без [$r[1]] тайник взломан силой — часть испорчена.", 'resourceEffects' => function($z,$l){return ['itemCount'=>rangeInt(1,2)];}, 'noResourceEffects' => function($z,$l){return ['itemCount'=>-1];}],
            ['text' => "Находим ценные сведения — карты, записи, координаты других схронов.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,4)];}, 'resourceCost' => $r[2], 'resourceText' => "[$r[2]] пригодились — контейнер распакован. Находка x3.", 'noResourceText' => "Без [$r[2]] контейнер взломан — часть испорчена.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,4),'itemCount'=>rangeInt(1,2)];}, 'noResourceEffects' => function($z,$l){return ['chips'=>NC($l,2)];}],
            ['text' => "Ловушка! Кто-то подстраховал находку. Получаем урон и теряем часть снаряжения.", 'weight' => 20, 'effects' => function($z,$l){return ['damagePercent'=>0.08,'chips'=>NC($l,0)];}, 'resourceCost' => $r[3], 'resourceText' => "[$r[3]] обезвредили ловушку — урон минимален.", 'noResourceText' => "[$r[3]] нет — ловушка сработала в полную силу.", 'resourceEffects' => function($z,$l){return ['damagePercent'=>-0.04];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>0.08];}],
            ['text' => "Забираем ценное, но приходится уходить под обстрелом.", 'weight' => 15, 'effects' => function($z,$l){return ['damagePercent'=>RF(0.03,0.06),'chips'=>C($l,3)];}, 'resourceCost' => $r[4], 'resourceText' => "Прикрываясь [$r[4]] уходим без царапины.", 'noResourceText' => "Приходится уходить под обстрелом без прикрытия — [$r[4]] нет, урон полный.", 'resourceEffects' => function($z,$l){return ['damagePercent'=>-0.03];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>RF(0.02,0.04)];}],
        ]];
    }
    if ($t === 'discovery') {
        $r = ['Вода', 'Батарейки', 'Лекарства', 'Консервы', 'Инструменты'];
        return ['prompt' => '', 'outcomes' => [
            ['text' => "Находка в целости — всё, что здесь было, достаётся нам.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,4),'exp'=>E($l,2),'itemCount'=>rangeInt(1,2)];}, 'resourceCost' => $r[0], 'resourceText' => "Благодаря [$r[0]] добираемся до тайника первыми — лут x3.", 'noResourceText' => "Без [$r[0]] упаковка не вскрыта — часть пропала.", 'resourceEffects' => function($z,$l){return ['itemCount'=>rangeInt(1,2),'chips'=>C($l,3)];}, 'noResourceEffects' => function($z,$l){return ['chips'=>NC($l,0)];}],
            ['text' => "Помимо основного, замечаем скрытый тайник в $zoneLabel.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,3),'itemCount'=>rangeInt(1,2)];}, 'resourceCost' => $r[1], 'resourceText' => "[$r[1]] помогают вскрыть тайник без шума — находка x2.", 'noResourceText' => "Без [$r[1]] тайник взломан силой — часть испорчена.", 'resourceEffects' => function($z,$l){return ['itemCount'=>rangeInt(1,2)];}, 'noResourceEffects' => function($z,$l){return ['itemCount'=>-1];}],
            ['text' => "Находим ценные сведения — карты, записи, координаты других схронов.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,4)];}, 'resourceCost' => $r[2], 'resourceText' => "[$r[2]] пригодились — контейнер распакован. Находка x3.", 'noResourceText' => "Без [$r[2]] контейнер взломан — часть испорчена.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,4),'itemCount'=>rangeInt(1,2)];}, 'noResourceEffects' => function($z,$l){return ['chips'=>NC($l,2)];}],
    ...
    if ($t === 'trade') {
        $r = ['Консервы', 'Лекарства', 'Батарейки', 'Топливо', 'Вода'];
        return ['prompt' => '', 'outcomes' => [
            ['text' => "Удаётся сторговаться по выгодной цене. Продавец доволен, мы при деньгах.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,2)];}, 'resourceCost' => $r[0], 'resourceText' => "Предложили [$r[0]] как оплату — продавец дал скидку. Прибыль x2.", 'noResourceText' => "Нет [$r[0]] для бартера — цена стандартная.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,5)];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Продавец оказывается щедрым — добавляет бонус к сделке.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,3)];}, 'resourceCost' => $r[1], 'resourceText' => "Обменяли [$r[1]] на редкий товар — бонус x2.", 'noResourceText' => "Без [$r[1]] бонус скромнее.", 'resourceEffects' => function($z,$l){return ['itemCount'=>1];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Обмениваемся знаниями и информацией о $zoneLabel.", 'weight' => 20, 'effects' => function($z,$l){return ['exp'=>E($l,4),'healPercent'=>RF(0.02,0.05)];}, 'resourceCost' => $r[2], 'resourceText' => "Поделились [$r[2]] — попутчики раскрыли секретные маршруты. Опыт x2.", 'noResourceText' => "Нечем поделиться — [$r[2]] нет. Поговорили и разошлись.", 'resourceEffects' => function($z,$l){return ['exp'=>E($l,4)];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Обман! Продавец подсовывает бракованный товар и скрывается с нашими чипами.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>NC($l,2),'damagePercent'=>RF(0.08,0.12)];}, 'resourceCost' => $r[3], 'resourceText' => "Вовремя заметили подмену — [$r[3]] спасли от обмана. Потери минимальны.", 'noResourceText' => "[$r[3]] нет — попались на удочку. Потери x2.", 'resourceEffects' => function($z,$l){return ['chips'=>0];}, 'noResourceEffects' => function($z,$l){return ['chips'=>NC($l,2)];}],
            ['text' => "Сделка проходит нейтрально — без прибыли, но и без потерь.", 'weight' => 15, 'effects' => function($z,$l){return ['chips'=>C($l,2),'exp'=>E($l,1)];}, 'resourceCost' => $r[4], 'resourceText' => "[$r[4]] подсластили сделку — выбили бонус.", 'noResourceText' => "Без [$r[4]] — ровно, как договорились.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,2)];}, 'noResourceEffects' => function($z,$l){return [];}],
        ]];
    }
    if ($t === 'story') {
        $r = ['Консервы', 'Вода', 'Лекарства', 'Топливо', 'Батарейки'];
        return ['prompt' => '', 'outcomes' => [
            ['text' => "Предлагаем помощь — нас благодарят и щедро вознаграждают.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'exp'=>E($l,3)];}, 'resourceCost' => $r[0], 'resourceText' => "Используем [$r[0]] чтобы помочь — благодарность x3.", 'noResourceText' => "Нечем помочь — [$r[0]] нет. Награда скромнее.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,5),'itemCount'=>rangeInt(1,2)];}, 'noResourceEffects' => function($z,$l){return ['chips'=>NC($l,1)];}],
            ['text' => "Рассказываем свою историю. Местные вдохновляются и делятся припасами.", 'weight' => 20, 'effects' => function($z,$l){return ['healPercent'=>RF(0.02,0.06),'exp'=>E($l,4)];}, 'resourceCost' => $r[1], 'resourceText' => "[$r[1]] как дар скрепляет дружбу — местные открывают запасы.", 'noResourceText' => "Без [$r[1]] история тронула, но припасов не дали.", 'resourceEffects' => function($z,$l){return ['itemCount'=>rangeInt(1,2),'chips'=>C($l,3)];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Помогаем в обмен на услугу. Взаимовыручка в $zoneLabel.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'itemCount'=>rangeInt(1,2)];}, 'resourceCost' => $r[2], 'resourceText' => "Оставляем [$r[2]] в залог — услуга оплачена вдвойне.", 'noResourceText' => "Без [$r[2]] услуга без благодарности.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,3)];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Проявляем наивность — нас обманывают и обкрадывают.", 'weight' => 20, 'effects' => function($z,$l){return ['damagePercent'=>RF(0.08,0.12),'chips'=>NC($l,2)];}, 'resourceCost' => $r[3], 'resourceText' => "[$r[3]] помогли сохранить часть припасов. Потери -50%.", 'noResourceText' => "Нас обманывают, [$r[3]] нет — потеряли всё.", 'resourceEffects' => function($z,$l){return ['chips'=>0];}, 'noResourceEffects' => function($z,$l){return ['chips'=>NC($l,2)];}],
            ['text' => "Помочь не вышло — ситуация обернулась против нас, но получили ценный урок.", 'weight' => 15, 'effects' => function($z,$l){return ['damagePercent'=>RF(0.02,0.04),'exp'=>E($l,4)];}, 'resourceCost' => $r[4], 'resourceText' => "[$r[4]] смягчили падение — легко отделались.", 'noResourceText' => "Помочь не вышло — [$r[4]] нет, урон вдвое сильнее.", 'resourceEffects' => function($z,$l){return ['damagePercent'=>-RF(0.01,0.02)];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>RF(0.02,0.04)];}],
        ]];
    }
    if ($t === 'danger') {
        $r = ['Инструменты', 'Изолента', 'Железо', 'Пластмасса', 'Дерево'];
        return ['prompt' => '', 'outcomes' => [
            ['text' => "Удача на нашей стороне — проходим опасный участок $zoneLabel без потерь.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,4),'exp'=>E($l,3)];}, 'resourceCost' => $r[0], 'resourceText' => "[$r[0]] помогают преодолеть препятствие — проходим с комфортом.", 'noResourceText' => "Без [$r[0]] идём на риск — проходим, но потрёпанные.", 'resourceEffects' => function($z,$l){return ['healPercent'=>RF(0.02,0.04)];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>RF(0.01,0.03)];}],
            ['text' => "Находим способ обезвредить угрозу — сообразительность спасает положение.", 'weight' => 20, 'effects' => function($z,$l){return ['exp'=>E($l,4),'chips'=>C($l,4)];}, 'resourceCost' => $r[1], 'resourceText' => "[$r[1]] — идеальный инструмент для обезвреживания. Награда x2.", 'noResourceText' => "Без [$r[1]] обезвреживаем подручными средствами.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,4),'itemCount'=>1];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Опасность закаляет дух. Выходим из передряги сильнее.", 'weight' => 20, 'effects' => function($z,$l){return ['healPercent'=>RF(0.02,0.05),'exp'=>E($l,4)];}, 'resourceCost' => $r[2], 'resourceText' => "[$r[2]] укрепляют позиции — меньше урона, больше опыта.", 'noResourceText' => "Без [$r[2]] выходим потрёпанными.", 'resourceEffects' => function($z,$l){return ['healPercent'=>RF(0.02,0.04),'exp'=>E($l,2)];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>RF(0.01,0.03)];}],
            ['text' => "Опасность оказалась сильнее. Получаем травмы и теряем часть снаряжения.", 'weight' => 20, 'effects' => function($z,$l){return ['damagePercent'=>0.12,'chips'=>NC($l,0)];}, 'resourceCost' => $r[3], 'resourceText' => "[$r[3]] спасают от худшего — травмы лёгкие.", 'noResourceText' => "Опасность сильнее, [$r[3]] нет — урон x2, потери серьёзные.", 'resourceEffects' => function($z,$l){return ['damagePercent'=>-0.06];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>0.12];}],
            ['text' => "Выбрались с трудом. Потрёпаны, но с добычей.", 'weight' => 15, 'effects' => function($z,$l){return ['damagePercent'=>RF(0.03,0.06),'chips'=>C($l,3)];}, 'resourceCost' => $r[4], 'resourceText' => "[$r[4]] помогли сохранить добычу — ценное уцелело.", 'noResourceText' => "Без [$r[4]] добыча рассыпалась — подобрали половину.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,3)];}, 'noResourceEffects' => function($z,$l){return ['chips'=>NC($l,0)];}],
        ]];
    }
    if ($t === 'heal') {
        $r = ['Дерево', 'Консервы', 'Вода', 'Лекарства', 'Изолента'];
        return ['prompt' => '', 'outcomes' => [
            ['text' => "Отличное место для отдыха. Восстанавливаем силы и дух.", 'weight' => 20, 'effects' => function($z,$l){return ['healPercent'=>RF(0.04,0.08),'exp'=>E($l,2)];}, 'resourceCost' => $r[0], 'resourceText' => "Разводим костёр с помощью [$r[0]] — отдых полноценный. Восстановление x2.", 'noResourceText' => "Без [$r[0]] отдых неполный.", 'resourceEffects' => function($z,$l){return ['healPercent'=>RF(0.04,0.06)];}, 'noResourceEffects' => function($z,$l){return ['healPercent'=>-RF(0.02,0.04)];}],
            ['text' => "Встречаем попутного лекаря. Он делится медикаментами и советами.", 'weight' => 20, 'effects' => function($z,$l){return ['healPercent'=>RF(0.03,0.07),'exp'=>E($l,3),'chips'=>C($l,3)];}, 'resourceCost' => $r[1], 'resourceText' => "Даём лекарю [$r[1]] — он делится лучшими препаратами. Лечение x3.", 'noResourceText' => "Нет [$r[1]] для обмена — помощь бесплатная, но скупая.", 'resourceEffects' => function($z,$l){return ['healPercent'=>RF(0.04,0.06)];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Обнаруживаем природный источник чистой воды и съедобные растения.", 'weight' => 20, 'effects' => function($z,$l){return ['healPercent'=>RF(0.02,0.05),'itemCount'=>rangeInt(1,2)];}, 'resourceCost' => $r[2], 'resourceText' => "С помощью [$r[2]] очищаем воду — запас пополнен.", 'noResourceText' => "Без [$r[2]] пьём как есть — риск заражения.", 'resourceEffects' => function($z,$l){return ['healPercent'=>RF(0.02,0.04)];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>RF(0.01,0.03)];}],
            ['text' => "Место оказалось заражённым. Вместо отдыха получаем отравление.", 'weight' => 20, 'effects' => function($z,$l){return ['damagePercent'=>0.08,'chips'=>NC($l,0)];}, 'resourceCost' => $r[3], 'resourceText' => "[$r[3]] нейтрализует яд — урон -50%.", 'noResourceText' => "Место заражённое, [$r[3]] нет — отравление в полную силу.", 'resourceEffects' => function($z,$l){return ['damagePercent'=>-0.04];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>0.08];}],
            ['text' => "Короткий привал — успеваем передохнуть, но в спешке срываем спину.", 'weight' => 15, 'effects' => function($z,$l){return ['healPercent'=>RF(0.01,0.03)];}, 'resourceCost' => $r[4], 'resourceText' => "[$r[4]] подкладываем под спину — отдых комфортный.", 'noResourceText' => "Без [$r[4]] спим на земле — спина болит.", 'resourceEffects' => function($z,$l){return ['healPercent'=>RF(0.02,0.04)];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>RF(0.01,0.02)];}],
        ]];
    }
    if ($t === 'neutral') {
        $r = ['Консервы', 'Вода', 'Батарейки', 'Топливо', 'Лекарства'];
        return ['prompt' => '', 'outcomes' => [
            ['text' => "Приятное знакомство у $zoneLabel. Новый друг — ценный союзник.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,4),'exp'=>E($l,2)];}, 'resourceCost' => $r[0], 'resourceText' => "Угощаем друга [$r[0]] — союзник делится припасами. Дружба x2.", 'noResourceText' => "Нечем угостить — [$r[0]] нет. Друг без подарков.", 'resourceEffects' => function($z,$l){return ['itemCount'=>1,'chips'=>C($l,2)];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Интересный собеседник делится ценной информацией о $zoneLabel.", 'weight' => 20, 'effects' => function($z,$l){return ['exp'=>E($l,4),'healPercent'=>RF(0.02,0.04)];}, 'resourceCost' => $r[1], 'resourceText' => "Платим [$r[1]] за информацию — данные подробнее и ценнее.", 'noResourceText' => "Без [$r[1]] информация общая, без деталей.", 'resourceEffects' => function($z,$l){return ['exp'=>E($l,4),'chips'=>C($l,3)];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Обмениваемся любезностями и подарками.", 'weight' => 20, 'effects' => function($z,$l){return ['chips'=>C($l,5),'itemCount'=>1];}, 'resourceCost' => $r[2], 'resourceText' => "Дарим [$r[2]] — взаимный обмен щедрый. Подарки x2.", 'noResourceText' => "Без [$r[2]] обмен скромный.", 'resourceEffects' => function($z,$l){return ['chips'=>C($l,3),'itemCount'=>1];}, 'noResourceEffects' => function($z,$l){return [];}],
            ['text' => "Незнакомец оказывается агрессивным. Приходится защищаться.", 'weight' => 20, 'effects' => function($z,$l){return ['damagePercent'=>RF(0.08,0.12),'chips'=>NC($l,0)];}, 'resourceCost' => $r[3], 'resourceText' => "[$r[3]] используем как щит — урон -50%.", 'noResourceText' => "Нет [$r[3]] — урон полный.", 'resourceEffects' => function($z,$l){return ['damagePercent'=>-RF(0.04,0.06)];}, 'noResourceEffects' => function($z,$l){return ['damagePercent'=>RF(0.04,0.06)];}],
            ['text' => "Нейтральное общение — ни тепло, ни холодно.", 'weight' => 15, 'effects' => function($z,$l){return ['exp'=>E($l,2)];}, 'resourceCost' => $r[4], 'resourceText' => "[$r[4]] как тема разговора — стало интереснее. Бонус.", 'noResourceText' => "[$r[4]] нет — разговор ни о чём.", 'resourceEffects' => function($z,$l){return ['exp'=>E($l,2),'chips'=>C($l,2)];}, 'noResourceEffects' => function($z,$l){return [];}],
        ]];
    }
    return null;
}
...
// ---------------------------------------------------------------------------
// Event category templates (server-side event texts)
// ---------------------------------------------------------------------------
$COMBAT_TEXTS = [
    ...
    "Группа вооружённых людей в масках выскакивает из засады. Треск автоматных очередей разрывает тишину.",
    "Звук мотора приближается — из-за поворота вылетает пикап с пулемётом. {faction} открывают огонь на поражение.",
    "Тишину разрезает окрик: «Стоять!». Из тумана выступают трое вооружённых {faction}. Главный целится тебе в голову.",
    "На пути — блокпост {faction}. За мешками с песком виднеется пулемёт. Договориться не выйдет — только бой.",
    "Грубый голос из темноты: «Живой товар!». Из засады вылетают {faction}. Главарь размахивает цепью. Бой неизбежен.",
    "Лай собак и топот сапог. Отряд {faction} заметил тебя. Командир отдаёт приказ: «Взять его!».",
    "Засада! Двое {faction} укрылись за старой цистерной. Один даёт очередь поверх головы.",
    "Патруль {faction} прочёсывает территорию. Заметив тебя, старший свистит — и вся группа разворачивается в твою сторону.",
    "«А ну стоять, мразь!» — рявкают из-за груды мусора. Рядом с окрикнувшим ещё пара {faction}.",
    "Ночной дозор {faction} обнаружил твой костёр. «Лёгкая добыча сама пришла», — слышен злобный шёпот.",
    "Из зарослей вылетает граната! Ты падаешь на землю — взрыв поднимает комья грязи. Из дыма выбегают {faction}.",
    "Снайпер! Пуля срывает кусок стены в паре сантиметров от головы. {faction} явно не хотят, чтобы ты прошёл.",
    "Тропу перегораживает завал из колючей проволоки. За ним — окоп {faction}. «Плати за проход или убирайся».",
    "Из подземного коллектора доносится лязг затвора. «Кто там?» — и в тебя летит связка «лимонок».",
];
$TRADE_TEXTS = [
    "На обочине расположился торговец {faction}. «Подходи, не стесняйся! Сегодня скидки для смелых!»",
    "Бродячий торговец катит тележку с деталями и консервами. «Меняю на чипы, патроны и истории».",
    "Торговец по имени {male} — здоровяк с татуировками — развернул палатку прямо посреди дороги.",
    "Хрупкая девушка {female} сидит на перевёрнутом ящике. Перед ней разложены самодельные бинты.",
    "Двое торговцев спорят о ценах на металл. Один из них манит рукой: «Помоги разгрузить — получишь долю».",
    "Маленькая тележка, запряжённая худой лошадью. Торговка {female} кричит: «Свежие новости и карты!»",
    "Молчаливый мужчина в чёрном плаще сидит у костра. Рядом — открытый чемодан с инструментами.",
    "На дереве висит самодельная вывеска «Лавка дядюшки Грыма». Хозяин подзывает тебя: «Гляди, что есть!»",
    "Странник в рваном плаще предлагает обменять инфу на припасы. «К востоку — заброшенный склад».",
    "Скупщик металлолома копается в груде ржавых запчастей. «Есть что продать? Или купить? Всё найду!»",
    "Бородатый мужик в кожаной безрукавке торгует с рук: «Лучшие чипы данных прямо с серверов Старого Мира!»",
    "Группа кочевников разбила юрты. Старейшина приглашает тебя на чай и обещает ценные сведения.",
];
$_UNUSED_HELP_TEXTS = [
    "Старик с замотанной рукой просит помощи. «Бандиты отобрали припасы, помоги отбить».",
    "Раненый сталкер лежит у обочины. «{faction} устроили облаву. Если есть аптечка, будь другом».",
    "На дороге сидит старик. Нога перевязана грязной тряпкой. «Сынок, помоги дойти до посёлка».",
    "Женщина с ребёнком на руках прячется в подвале разрушенного дома. «Муж ушёл за водой и не вернулся…»",
    "Молодой парень копается в двигателе старого грузовика. «Не заведётся, чтоб его!».",
    "Маленький мальчик сидит на обочине и плачет. «Я потерялся…»",
    "Пожилая пара пытается перетащить телегу через завал. Женщина просит: «Помоги, сынок».",
    "Юноша сидит на корточках у капкана, в который попался его друг. «Отцепи его, прошу!»",
    "Девушка в запылённой куртке чинит солнечную панель на крыше сарая. «Подай ключ на 10!»",
    "Семья переселенцев остановилась у ручья. Отец чинит телегу: «Не подкинешь бензина?»",
    "Двое сталкеров спорят, куда идти. Один говорит: «Налево — к базе, направо — к смерти».",
    "Из ямы кричит мужчина: «Помогите! Сорвался, ногу подвернул!»",
];
$TRAP_TEXTS = [
    "Осторожно! Под ногой что-то щёлкает. Растяжка! Замираем. Проволока натянута у самой щиколотки.",
    "Земля подозрительно свежая. Яма-ловушка с кольями на дне. {faction} промышляют разбоем.",
    "Мужчина с перевязанным глазом машет: «Помоги, брат, друзей в засаде бросили!» Ведёт прямиком в ловушку.",
    "Девушка с заплаканным лицом просит воды. Ты протягиваешь флягу — она выбивает её из рук.",
    "Запах дыма и жареного мяса. Компания машет: «Садись, путник, угощайся!» Мясо отравлено.",
    "Яркая тряпка на ветке — указатель к «Бесплатному складу». Там — растяжка с гранатой.",
    "«Эй, чувак, хочешь дешёвый ствол?» — парень достаёт пистолет-муляж, подельник обчищает рюкзак.",
    "Из темноты — крик о помощи. Бежишь на звук и проваливаешься в яму-ловушку, прикрытую ветками.",
    "На земле валяется дорогой артефакт. Слишком удачно. Точно приманка. Вокруг — следы засады.",
    "Привал у ручья. Вода пахнет металлом — кто-то отравил источник выше по течению.",
];
$LOOT_TEXTS = [
    "Находим брошенный рюкзак. Внутри — консервы и патроны. Удача!",
    "Старый сейф в углу разрушенного здания. Замок сбит, но внутри ещё есть ценное.",
    "В разрушенном доме слышны голоса. Ты пытаешься обойти — но натыкаешься на склад припасов.",
    "Под обломками блестит что-то металлическое. Разбираешь завал и находишь ящик с инструментами.",
    "Труп сталкера в странной позе. В рюкзаке — карта с пометками и пачка чипов.",
    "Старый тайник в дупле огромного дерева. Кто-то спрятал здесь запасы на чёрный день.",
    "Разбитый военный грузовик. В кузове — уцелевшие ящики с патронами и сухпайками.",
    "Сейф в подвале сгоревшего магазина. Внутри — довоенные монеты и ювелирка.",
    "Сбитый беспилотник. В памяти — координаты других точек с припасами.",
    "Рюкзак, висящий на ветке. Кто-то прятал его от зверей. Внутри — консервы и бинты.",
];
$DISCOVERY_TEXTS = [
    "Странная конструкция из металлолома и проводов. Похоже на самодельный генератор.",
    "Лаз в подземелье, скрытый кустарником. Ступеньки уходят в темноту. Снизу тянет холодом.",
    "Вход в старый бункер. Дверь массивная, но приоткрыта. Внутри темно и тихо.",
    "Заброшенная лаборатория. На столах — колбы, провода, записи. Кто-то ставил опыты.",
    "Радиостанция старого образца. Она всё ещё работает! Можно поймать сигнал.",
    "Старая карта, прибитая к стене. На ней отмечены маршруты контрабандистов.",
    "Подземный переход затоплен. В мутной воде угадываются очертания чего-то ценного.",
    "Ржавая лестница ведёт на крышу небоскрёба. Оттуда открывается вид на весь район.",
    "Замурованная ниша в стене. Кладка свежая — кто-то прятал здесь что-то недавно.",
    "Остов старого танка. Внутри тесно, но уцелел прицел и пара снарядов (не взрываются).",
    "Пещера за водопадом. Внутри сухо и есть следы костра. Кто-то здесь жил.",
];
$ANOMALY_TEXTS = [
    "Воздух дрожит и потрескивает. Впереди — гравитационная аномалия. Камни парят в воздухе.",
    "Странное свечение исходит из центра поляны. Аномалия пульсирует, притягивая металл.",
    "Жар-птица: золотисто-багровое марево, выжигающее всё вокруг. Внутри угадывается артефакт.",
    "Электра: воздух пронизывают голубые разряды, волосы встают дыбом. Очень опасно.",
    "Комариная плешь: участок мёртвой земли, где даже трава не растёт. Счётчик зашкаливает.",
    "Холодец: прозрачная студенистая масса, пульсирующая в такт сердцу. Тянется к живому.",
    "Трава на поляне выжжена правильным кругом. Камни в центре светятся тусклым зелёным.",
    "В воздухе висит тонкая серебристая пыль. Она мерцает в лучах солнца. Красиво, но дышать тяжело.",
];
$NPC_TEXTS = [
    "Одинокий путник {nick} сидит у костра. «Заходи, путник, места хватит. Давно не видел живых».",
    "Маленький посёлок. Дома сколочены из обломков, в окнах горит свет. Пахнет хлебом.",
    "Встречаешь отшельника {male}. Он живёт в землянке и разводит кроликов. «Здесь тихо, бери пример».",
    "Старуха-знахарка собирает травы на опушке. «Помоги собрать, внучок, отблагодарю».",
    "Бродячий музыкант играет на губной гармошке у сгоревшего дома. Меланхоличная мелодия.",
    "Дети играют в «войнушку» возле сгоревшего бронетранспортёра. Они не видели чужаков с рождения.",
    "Монах в оранжевой робе медитирует под палящим солнцем. У него закончилась вода.",
    "Слепой ветеран сидит у могильного креста. «Сын здесь лежит. Не уберёг…»",
    "Молодая пара строит дом из обломков. «Не армия, не банда — просто хотим жить по-человечески».",
    "Учитель в развалинах школы проводит урок для горстки детей. «Хочешь, посиди, послушай».",
];
$REST_TEXTS = [
    "Уютная поляна среди деревьев. Ручей журчит рядом. Отличное место для привала.",
    "Заброшенная изба. Крыша цела, печь работает. Можно переночевать в тепле и безопасности.",
    "Поляна диких цветов простирается до горизонта. Ветер колышет траву. Благодать.",
    "Пещера с выходом к горному озеру. Вода прозрачная, рыба плещется у берега.",
    "Старая сторожевая башня. Внутри сухо, есть лежанка и запас дров. Настоящий курорт.",
    "Парники с уцелевшими стёклами. Внутри тепло и влажно, можно переночевать среди зелени.",
    "Остановка общественного транспорта. Скамья цела, навес держится. Место для отдыха.",
    "Стог сена в чистом поле. Мягко, тепло, пахнет летом. Идеально для дневного сна.",
];

// ---------------------------------------------------------------------------
// Help/story templates — rich objects with hand-crafted branches
// ---------------------------------------------------------------------------
$HELP_TEXTS_RICH = [
    [
...
// ---------------------------------------------------------------------------
// New categories from client
// ---------------------------------------------------------------------------
$FACTION_TEXTS = [
    "Отряд {faction} проводит учения на окраине. Командир машет рукой: «Не отставай!» — принимает тебя за своего.",
    "Лагерь {faction} разбит в старом ангаре. Часовые играют в карты. В пристройке находишь забытый рюкзак.",
    "Двое {faction} не поделили добычу. Пока они выясняют отношения, проскальзываешь мимо.",
    "Патруль {faction} прочёсывает местность. Прячешься в старой цистерне. После ухода находишь ящик.",
    "Мародёры в форме {faction} обыскивают трупы. Принимают тебя за своего. «Обыщи тех двоих, быстро!»",
    "Представители {faction} вербуют рекрутов. «Вступай — паёк, патроны, крыша над головой!»",
    "Радиоперехват: {faction} вызывают подкрепление. «На секторе 7 — одиночка. Возьмите живым».",
    "Техника {faction} застряла в болоте. Водитель матерится: «Помоги — заплачу».",
    "Пленный {faction} сидит в яме. «Вытащи — отблагодарю». Вытаскиваешь — он убегает, бросив кошелёк.",
    "Штаб {faction} в старом здании. Вокруг патрули, прожектора, пулемётные гнёзда.",
    "«Стой! Пропуск!» — окрик {faction}. Пропуска нет. «Давай 20 чипов, так и быть, пропущу».",
    "{faction} устроили зачистку мутантов. После боя забирают трофеи. Остаётся один «забытый» ствол.",
    "Пьяный боец {faction} сидит в канаве. «Брат, налей!» Даёшь воды — он дарит золотой зуб на удачу.",
];
$SPECIAL_TEXTS = [
    "Из тумана выходит фигура в плаще. «Ты ищешь. Я знаю. Ступай за мной». Ведёт к тайнику с артефактом.",
    "Огненный шар падает с неба. Через час добираешься до места падения — обломки спутника с данными.",
    "Подземный город! Целый этаж уцелел: стеклянный купол, деревья в кадках. Люди живут здесь десять лет.",
    "«Бешеный» медведь-мутант выходит на поляну. Три метра ростом. Фыркает, разворачивается и уходит.",
    "НЛО? В небе зависает светящийся треугольник. Исчезает со сверхзвуковым хлопком. В центре — кристалл.",
    "Работающий терминал Старого Мира. На экране — просьба о помощи десятилетней давности. И код доступа.",
    "Боевой дрон активируется: «Цель — гражданский. Миссия отменена». Падает к ногам — сел аккумулятор.",
    "Зеркальный пруд: гладь неподвижна. В отражении видишь свою версию из «хорошего» мира.",
    "Радиоактивный дождь. Укрываешься в машине. Находишь на земле светящиеся капли — «звёздную пыль».",
    "На перекрёстке стоит указатель с именами и датами. «Артём — 2034», «Мария — 2037». Кто-то помнит павших.",
    "В разрушенном парке аттракционов крутится карусель. Электричества нет. Волосы шевелятся.",
    "Из-под обломков доносится стук. Разбираешь завал — сейф с детскими рисунками и пачкой чипов.",
];
$BRANCHING_TEXTS = [
    "Тропа раздваивается: налево — через тёмный лес {zone}, направо — вдоль высохшего русла реки.",
    "Замечаешь свежие следы на грязи. Кто-то прошёл здесь недавно. Следы ведут к старому бункеру.",
    "Старый мост через реку. Перила сгнили, доски прогнили. Снизу — мутная вода и коряги.",
    "Находишь работающий генератор. Он гудит и подрагивает, вырабатывая электричество для целой улицы.",
    "Проходишь мимо сгоревшего БТРа. Внутри обгоревшие останки и уцелевший ящик.",
    "Лесная тропа выводит к старой охотничьей заимке. Из трубы идёт дым — там кто-то есть.",
    "На поляне стоит странное сооружение из костей и проволоки. Похоже на тотем или предупреждение.",
    "Слышен шум воды — водопад. За ним угадывается вход в пещеру. Вода холодная и чистая.",
    "Старый колодец. Ведро есть, верёвка цела. Внутри плещется вода — пахнет сыростью.",
    "На обочине валяется велосипед. Рама цела, одно колесо спущено. Можно починить.",
    "Радиомачта. Забираешься наверх — открывается вид на весь район. Видно несколько точек интереса.",
    "Заброшенная автозаправка. Под прилавком — ящик с консервами и пыльная бутылка водки.",
    "Груда металлолома. Среди ржавых деталей поблёскивает что-то ценное. Надо покопаться.",
    "Бродячая собака крутится у развалин. У неё в зубах — человеческая кость. Жутковатое зрелище.",
    "Старый бронепоезд стоит на рельсах. Внутри темно, но слышны звуки — кто-то там живёт.",
    "Проходишь мимо кладбища старой техники. Экскаваторы, бульдозеры, грузовики — кладбище великанов.",
    "Одинокий дом на холме. Стреляют изнутри — кто-то обороняется от мутантов. Надо помочь или пройти мимо.",
];
